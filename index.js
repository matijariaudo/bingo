...
app.post('/user/img',async(req,res)=>{
  const img=await bingoModel.find();
...
app.post('/upload', upload.single('archivo'),async(req, res) => {
  ...
  if (req.file) {
    ...
    const archivo_name = req.body.archivo_name;
    ...
    const img=new bingoModel({url:name,name:archivo_name})
    let f=0; let c=0;
    contenido.forEach(t => {
      t.forEach(e => {
        console.log(e,{song:e,fila:f,columna:c})
